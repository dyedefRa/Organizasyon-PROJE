...
using Organizasyon.Models.MailModels;
...
namespace Organizasyon.Services
{
    ...
    public class MailTemplateAppService : CrudAppService<MailTemplate, MailTemplateDto, int, PagedAndSortedResultRequestDto, MailTemplateDto, MailTemplateDto>, IMailTemplateAppService
    {
        ...
        {
            string footerHtml = "";
            var mailTemplate = await GetMailTemplateByMailKey(OrganizasyonConsts.MailTemplateKey.TemplateFooter);
        ...
        private async Task<MailResultModel> GetMailTemplateBody(string key)
        {
            ...
            var mainTemplate = await GetMailTemplateByMailKey(OrganizasyonConsts.MailTemplateKey.TemplateBody);
        ...
        public async Task<bool> SendNewPasswordMailAsync(string passwordChangeUrl, string toMail)
        {
            try
            {
                var mailTemplate = await GetMailTemplateBody(OrganizasyonConsts.MailTemplateKey.NewPassword);
